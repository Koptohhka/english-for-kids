(() => {
    ...
    let renderPlayCardsEventFunction = (evt) => {
        let targetElement = evt.target;
    ...
    let renderStatycticsTable = (data) => {
        let rowsArray = [];
        for (item in data) {
            ...
            data[item].forEach((it) => {
                ...
                let rowItem = `<tr class="card-list__table-row">${dataCelssArray.join('')}</tr>`;
            });
        }

        //mainCardsContainer.innerHTML = `<table class="card-list__table">${rowsArray.join('')}</table>`;
    }
    //renderStatycticsTable(window.data.playCardsData);

    /*
    let dataCelssArray = [];    

            data[item].forEach((it) => {
                let dataCellItem = `<td class="card-list__table-data">Mock</td>`;
                dataCelssArray.push(dataCellItem);
            });
            let rowItem = `<tr class="card-list__table-row">${dataCelssArray.join('')}</tr>`;
            console.log(data[item]);
            rowsArray.push(rowItem);
    */
